...
import { ArrowRightCircleIcon, InformationCircleIcon, XCircleIcon } from '@heroicons/react/24/outline'

const faqList = [
  {
    id: 1,
    question: 'What is Swasthya?',
    answer: 'Swasthya is a secure, modern and unified platform for patients and doctors.'
  },
  {
    id: 2,
  ...
  return (
    <div className='fixed bottom-10 right-6 z-40 flex flex-col space-y-2 transition-all duration-400 ease-in-out'>
      <button
        ...
        className={`w-10 h-10 p-1 bg-teal-600 cursor-pointer scale-on-hover hover:shadow-md hover:shadow-teal-100 ${isOpen ? 'self-end rounded-br-2xl rounded-tl-2xl' : 'rounded-tr-2xl rounded-bl-2xl'}`}
      >
        {isOpen ? (
